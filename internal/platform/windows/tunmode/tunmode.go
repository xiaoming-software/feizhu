...
func Start(ctx context.Context, cfg Config) (*Controller, error) {
	...
	// 上次异常退出、睡眠唤醒或网络切换后，/1 默认路由可能仍绑在旧 TUN 上。
	// 在管理员 helper 内启动前先兜底清理，避免必须重启 Windows 才恢复。
	_, _ = CleanupStale()
...
func (c *Controller) refreshRoutesIfNeeded() {
	...
	log.Printf("[TUN] 检测到默认网络变化，刷新 Windows TUN 路由：%s/%s -> %s/%s", c.route.Gateway, c.route.Interface, next.Gateway, next.Interface)
