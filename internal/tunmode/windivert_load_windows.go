...
func ensureWinDivertLoaded() error {
	...
	if err != nil {
		return fmt.Errorf("tunmode: 无法定位程序目录: %w", err)
	}
	dir, err := filepath.Abs(filepath.Dir(exe))
	if err != nil {
		return fmt.Errorf("tunmode: 无法解析程序目录: %w", err)
	}
	...
	if err := godivert.LoadDLL(dllPath, dllPath); err != nil {
		return fmt.Errorf("tunmode: 加载 WinDivert.dll 失败: %w", err)
	}
	if err := godivert.CheckDLL(); err != nil {
		return fmt.Errorf("tunmode: WinDivert 驱动未就绪（需管理员权限）: %w", err)
	}
	return nil
}
...
func materializeWinDivertFile(path string, data []byte) error {
	if len(data) == 0 {
		if _, err := os.Stat(path); err != nil {
			return fmt.Errorf("tunmode: 未找到 %s，请使用 build.sh 编译 Windows 版", path)
		}
		return nil
	}
	...
	if err := os.WriteFile(path, data, 0644); err != nil {
		return fmt.Errorf("tunmode: 释放 %s 失败: %w", filepath.Base(path), err)
	}
	return nil
}
