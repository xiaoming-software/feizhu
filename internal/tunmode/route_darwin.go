...
`, strings.Join(bypass, ", "), c.DeviceName, peer.String())
	...
	if err := runDarwin("pfctl", "-a", "com.apple/feizhu", "-f", path); err != nil {
		return fmt.Errorf("tunmode: 加载 TCP-only pf 规则失败: %w", err)
	}
	if err := runDarwin("pfctl", "-E"); err != nil && !strings.Contains(err.Error(), "pf already enabled") {
		return fmt.Errorf("tunmode: 启用 pf 失败: %w", err)
	}
	return nil
}
...
func darwinDefaultRoute() (net.IP, string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("tunmode: 获取默认路由失败: %w", err)
	}
	var gw net.IP
	var iface string
	...
	if gw == nil || iface == "" {
		return nil, "", fmt.Errorf("tunmode: 无法解析默认路由 gateway/interface")
	}
	return gw, iface, nil
}
